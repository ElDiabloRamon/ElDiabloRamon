...
sudo apt -y install htop mc netcat screen hexedit nmon iptraf aide python3 ruby perl gcc p7zip rar gzip mlocate ncftp lynx elinks wget curl clamav ngrep vim alpine mutt snapd tlp tlp-rdw timeshift cowsay fortune-mod lolcat figlet toilet trash-cli chafa cmatrix sl ntp ntpdate;
...
sudo apt -y install gimp inkscape blender vlc synaptic gnome-tweak-tool gufw openjdk-11-jdk gkrellm openbox obconf tint2 conky feh lightdm guake putty filezilla xcompmgr cairo-dock simplescreenrecorder openarena openshot-qt seahorse-nautilus net-tools lynis;
