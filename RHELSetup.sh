...
#sudo yum check-update;yum -y update;

#yum check-update;

sudo yum -y install net-tools rsyslog curl mlocate strace ltrace libpcap htop lynx elinks dig traceroute dstat ccze pv pdsh nmon pstree openssh openssl rsync mtr nc mc iptraf hexedit p7zip ngrep tcpdump nmap vmstat psacct acct  nmon zip unzip python3 python-pip python-construct python-setuptools pylibpcap scapy python-paramiko python-crypto python-ecdsa PyYAML net-tools lshw NetworkManager-tui lrzsz;
...
sudo systemctl status alsa-state.service;                  # loaded active running Manage Sound Card State (restore and store)
...
sudo systemctl status bluetooth.service;                  # loaded active running Bluetooth service

sudo systemctl disable bluetooth.service;


sudo systemctl status wpa_supplicant.service;             # wifi daemon.
